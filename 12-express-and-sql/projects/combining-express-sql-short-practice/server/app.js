 ...
 */
// Your code here
const sqlite3 = require('sqlite3'); // Require sqlite3
...
app.get('/colors/add/:name', (req, res, next) => {
    ...
    db.run(sql, params, function (err) {
        if (err) {
            next(err); // Call Express error handling middleware
            return;
        }

        // Query for the last inserted color
        db.get(sqlLast, (err, row) => {
            ...
            // Respond with the last color in JSON format
            res.json(row);
        });
    });
})
